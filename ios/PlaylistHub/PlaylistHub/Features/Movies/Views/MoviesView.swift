...
struct MoviesView: View {
                ...
                Group {
                    if vm.isLoading && vm.groupedItems.isEmpty && vm.items.isEmpty {
                        ...
                        EmptyStateView(
                            icon: "film",
                            title: "No playlists",
                            subtitle: "Add a playlist to browse movies."
                        )
                        ...
                        EmptyStateView(
                            icon: "film",
                            title: "No movies",
                            subtitle: "This playlist doesn't contain any movies yet."
                        )
                        .frame(maxWidth: .infinity, maxHeight: .infinity)
                    } else if !vm.searchText.isEmpty {
                        posterGridView
                    } else if vm.groupedItems.count == 1, let group = vm.groupedItems.first {
                ...
                ForEach(vm.playlists) { playlist in
                    Button {
                        ...
                        HStack(spacing: 5) {
                            Text(playlist.name)
                                .lineLimit(1)
                            if playlist.moviesCount > 0 {
                                Text("\(playlist.moviesCount.abbreviated)")
    ...
    // MARK: - Poster Grid (search / single group)

    private var posterGridView: some View {
        posterGridView(items: vm.items)
    }
